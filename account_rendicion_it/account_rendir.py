...
class account_rendicion_it(models.Model):
	...
	@api.one
	def get_saldo(self):
		self.saldo = self.monto_entregado - self.monto_rendido

	@api.one
	def get_monto_rendido(self):
		param = self.env['main.parameter'].search([])[0]
		flag = True if self.caja_entrega.currency_id.name == 'USD' else False
		moves = self.env['account.move'].search([('rendicion_id','=',self.id),('state','!=','draft')])
		account = param.deliver_account_me.id if flag else param.deliver_account_mn.id
		tot = 0
		for move in moves:
			for line in move.line_ids:
				if line.account_id.id == account:
					tot += line.amount_currency if flag else line.credit
	...
	def get_monto_entregado(self):
		...
		for i in lineas:
			if i.account_id.id == (param.deliver_account_me.id if flag else param.deliver_account_mn.id) and i.debit > 0:
				if flag:
					tot += i.amount_currency if flag else i.debit

		tc = self.tc_oficial
		if self.tc_personalizado:
			tc = self.tc_personalizado

		self.monto_entregado = tot+self.monto 
