        ...
        if sequence.use_date_range:
            date_range = self.env["ir.sequence.date_range"].search(
                [("sequence_id", "=", sequence.id),
                 ...
                 ("date_to", ">=", self.periodo.date_stop)],limit=1)

            if not date_range:
            	raise UserError(u'No se ha encontrado una secuencia de rango de fechas para el diario %s, es posible que no haya generado la secuencia de dicho diario.'%self.journal_id.name)
            elif len(date_range)>1:
                raise exceptions.MissingError( _('Existe dos intervalos de fecha que se cruzan en el diario ' + self.journal_id.name))
            else:
                date_range.number_next_actual = default
                date_range.number_next = default
