...
class MultipaymentadvanceInvoice(models.Model):
	...
	td_caja = fields.Many2one('einvoice.catalog.01','Tipo Documento Caja')
	partner_caja = fields.Many2one('res.partner','Partner para Caja')
	...
	def crear_asiento(self):
		...
		for i in self.asiento.line_ids:
			if self.journal_id.default_debit_account_id.id == i.account_id.id :
				if self.td_caja.id:
					i.type_document_it = self.td_caja.id
				if self.partner_caja.id:
					i.partner_id = self.partner_caja.id
