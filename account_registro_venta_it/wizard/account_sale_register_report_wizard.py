...
class account_sale_register_report_wizard(osv.TransientModel):
	...
	def reporteador(self):		
		...
		elements.append(Paragraph(texto, style_form))
		elements.append(Spacer(1, 12))
		texto = 'Ruc: ' + (company.nro_documento)
		elements.append(Paragraph(texto, style_form))
		elements.append(Spacer(1, 12))
		texto = 'Registro Ventas: ' + (self.period_ini.name) + ' - ' + self.period_end.name
		elements.append(Paragraph(texto,style_form))
		elements.append(Spacer(1, 12))
	...
	#Crear Tabla
		...
		for fila in dicc:
			...
			tipodoc = (fila['tipodoc']) if fila['tipodoc'] else ''
			numdoc = (fila['numdoc']) if fila['numdoc'] else ''
			...
			seriemod = (fila['seriemod']) if fila['seriemod'] else ''			
			numeromod = (fila['numeromod']) if fila['numeromod'] else ''
