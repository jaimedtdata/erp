...
class PlanillaPlanillaTabularWizard(models.TransientModel):
    ...
    def reconstruye_tabla(self,date_start,date_end):
        ...
        inner join hr_structure_salary_rule_rel srl
        on srl.rule_id = sr.id
        inner join hr_payroll_structure hps
        on hps.id=srl.struct_id
        where srl.struct_id=%d and sr.appears_on_payslip='t'
        ...
        SELECT * FROM crosstab(
            $$ 
                select e.name_related,e.identification_id,pa.entidad,hpl.sequence,hpl.total from hr_payslip hp
                inner join hr_payslip_line hpl
                on hp.id=hpl.slip_id
                inner join hr_employee e 
                on e.id = hpl.employee_id
                inner join hr_contract hc
                on hc.employee_id = e.id
                inner join planilla_afiliacion pa
                on pa.id = hc.afiliacion_id
        ...
        """ % (date_start, date_end, structure_data['id'], fields)
