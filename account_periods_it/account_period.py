...
class account_period(models.Model):
	...
	def create(self,vals):
		t = super(account_period,self).create(vals)
		if len( self.env['account.period'].search([('name','=',t.name),('id','!=',t.id)]) )>0:
			raise UserError(u'Ya existe un Periodo con ese mismo nombre.')
		return t


	@api.one
	def write(self,vals):
		t = super(account_period,self).write(vals)
		self.refresh()
		if len( self.env['account.period'].search([('name','=',self.name),('id','!=',self.id)]) )>0:
			raise UserError(u'Ya existe un Periodo con ese mismo nombre.')
		return t