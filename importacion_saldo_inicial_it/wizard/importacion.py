...
class wizard_import_asiento_apertura(models.Model):
	...
	def importar(self):
		...
		self.env.cr.execute("""
			...
			select '"""+str(self.fecha_contable)+"""',fecha_emision, campo1 ||'-' || tipo_doc ||'-' || numero || '-' || cuenta , 'posted',"""+str(self.diario.id)+""", numero,1, '1',"""+str(self.id)+""","""+('true' if ( str(self.fecha_contable) == '-01-01') else 'false' )+""" from import_asiento_apertura
			where wizard_id = """ +str(self.id)+ """;		
		...
		if True:
			self.env.cr.execute(""" 
				...
				from import_asiento_apertura iaa
				inner join account_move am on am.name = campo1 ||'-' || iaa.tipo_doc || '-' || iaa.numero || '-' || iaa.cuenta  and am.apertura_id = """ +str(self.id)+ """
		...
		if self.tipo =='1':
			self.env.cr.execute("""
				INSERT INTO account_invoice(date_due,number,partner_id,it_type_document,reference,
				...
				select iaa.fecha_vencimiento, iaa.numero, iaa.razon_social, iaa.tipo_doc, iaa.numero,
