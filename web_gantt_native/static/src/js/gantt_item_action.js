odoo.define('web_gantt_native.ItemAction', function (require) {
    ...
    var GanttListAction = Widget.extend({
        ...
        start: function () {
            ...
            _.each(nodes, function (node) {
                ...
                _.each(childNodes, function (child) {
                    ...
                    if (child["date_done"]) {
                        item_action.append('<span class="action-done"><i class="fa fa-check-circle-o"></i></span>');
                    }
        ...
        focus_gantt_line: function (event) {
            ...
            if (!record.is_group) {
                var toscale = Math.round((record.task_start.getTime() - self.firstDayScale) / self.pxScaleUTC);
                ...
                self.gantt_timeline_scroll_widget.scrollOffset(new_toscale);
