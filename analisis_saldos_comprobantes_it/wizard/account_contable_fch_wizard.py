...
fecha.date_due as vencimiento,
CASE WHEN current_date - fecha.date_due >0 THEN current_date - fecha.date_due ELSE 0 END as dias_atraso
from (
...
left join (
	select am.id, max(aml.date_maturity) as date_due from account_move am
	inner join account_move_line aml on aml.move_id = am.id
	group by am.id
) fecha on fecha.id = am.id
order by  code,empresa, nro_comprobante
		...
		if self.mostrar == 'excel':
			...
			worksheet.write(9,11, u"Vendedor",boldbord)
			worksheet.write(9,12, u"Fecha Vencimiento",boldbord)
			worksheet.write(9,13, u"Dias de Atraso",boldbord)
			...
			for line in self.env['saldo.comprobante'].search(filtro):
				...
				worksheet.write(x,11,line.vendedor if  line.vendedor else '',bord)
				worksheet.write(x,12,line.vencimiento if  line.vencimiento else '',bord)
				worksheet.write(x,13,line.dias_atraso ,numberdos)
