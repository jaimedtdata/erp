...
class ProjectTaskNativeScheduler(models.Model):
        ...
        if not task.child_ids:  # (task not group.)
            ...
            if task.schedule_mode == "auto":  # (auto mode)
                ...
                if new_date is None:
                    return tasks
                    # raise UserError(_(
                    #     "New Date {} - not set in project".format(date_type)
                    # ))
    ...
    def _ap_calc_period(self, task_obj, direction, new_date, date_type, t_params, cal_value=None):
        ...
        if task_obj:
            ...
            if calendar_level:
                date_start = self._get_date_from_level(calendar_level, cal_value["start_value"],
                                                       cal_value["start_type_op"])
    ...
    def _calc_date_list(self, scheduling_type, predecessors, tasks, predecessor_obj):
        ...
        search_date_objs = filter(lambda x: x[dt_list_task_field] == dt_list_task_id and x['type'] == type_link,
                                  predecessors)
