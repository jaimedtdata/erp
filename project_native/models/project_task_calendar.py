...
class ProjectTaskNativeCalendar(models.Model):
    ...
    def _push_interval_log(self, level, global_leave_ids_param, attendance, date_from, date_to, iteration,
                           res_id, cal_id, flag_task, direct="normal"):
    ...
    def check_load_control(self, _load_control, _calendar_id, _resource_id, global_leave_ids_param, direct, flag_task):
        ...
        global_leave_ids = list(filter(lambda x: x.get("flag_task") and x["flag_task"] != flag_task, global_leave_ids))
    ...
    def _get_diff_e_counter(self, task_res, fixed_calc_type, diff):
        ...
        for _res in task_res:
            ...
            if fixed_calc_type == "duration":
                diff_e_counter[_resource_id] = timedelta(seconds=diff_k.total_seconds() * _load_factor)
    ...
    def _get_work_intervals(self, _resource_id, work_already_day, work_time_day, _load_factor):
        ...
        # python3
        work_time = timedelta(seconds=work_time_day[_resource_id].total_seconds() * _load_factor)
